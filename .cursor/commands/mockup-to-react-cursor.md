...
# Workflow: Mockup to React Implementation (Cursor-Compatible)
...
1.  **Read the Mockup**: Use `read_file` to read the content of the provided mockup file.
...
2.  **Browser Test**:
    -   Run the development server using `run_terminal_cmd` with `npm run dev` in the `gem-react-library` directory.
    -   Manually verify the implementation in the browser.
